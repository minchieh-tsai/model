...
if __name__ == "__main__":
    model_ft = build_resunetplusplus()

    inputs = torch.ones([1, 3, 1024, 1024])
    outputs = model_ft(inputs)
    print('outputs', outputs[0, 0, 0, 0:100])

    traced_script_module = torch.jit.trace(model_ft, inputs)
    traced_script_module.save("./ResUnet_plus/model_trace_v2.pt")

    # loaded = torch.jit.load("model_trace.pt")
    # print(loaded)
    # print(loaded.code)